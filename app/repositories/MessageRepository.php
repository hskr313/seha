...
class MessageRepository extends BaseRepository {
    ...
    public function findByConversation(int $user1_id, int $user2_id): array {
        ...
        return array_map([$this, 'mapToEntity'], $result->fetch_all(MYSQLI_ASSOC));
    }
    ...
    public function countUnreadMessages(int $userId): int {
        $query = "SELECT COUNT(*) AS unreadCount FROM {$this->table} WHERE receiver_id = ? AND read_at IS NULL";
        $stmt = $this->db->prepare($query);
        $stmt->bind_param('i', $userId);
        $stmt->execute();
        $result = $stmt->get_result();
        $row = $result->fetch_assoc();
        return (int) $row['unreadCount'];
    }
}
